...
Specifically, we implemented several methods for that purpose:
 -  online learning algorithms for recurrent neural networks (RNN), for sequence-specific training (MATLAB)
 ...
 - encoder-only transformers (both population-based and sequence-specific models) trained with backpropagation (Pytorch, Optuna; code interfaced with MATLAB for evaluation)
...
The repository is essentially split into two different folders:
 1. The "Time_series_forecasting" folder contains scripts and functions that perform time series forecasting. It is essentially self-contained / independent, and is an extension of [one of our previous respositories](https://github.com/pohl-michel/time-series-forecasting-with-UORO-RTRL-LMS-and-linear-regression) [[associated research article]](https://doi.org/10.48550/arXiv.2106.01100).
...
Prior to using transformers, one need to install the Pytorch library and [set the Python interpreter path in Matlab](https://uk.mathworks.com/help/matlab/ref/pyenv.html). For instance, if using conda/miniconda:
