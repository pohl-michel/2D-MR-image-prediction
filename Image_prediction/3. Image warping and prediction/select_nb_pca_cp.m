function [eval_results, best_pred_par_struct, best_pca_cp_tab] = select_nb_pca_cp(beh_par, disp_par, OF_par, im_par, path_par, br_model_par, eval_results, warp_par)
   ...
   hppars = load_hyperpar_cv_info( pred_par );
    ...
    for nb_pca_cp=1:nb_pca_cp_max
        ...
        % Computation of PCA [I returned W & F for debugging purposes]
        ...
        beh_par.SAVE_PREDICTION_PLOT = false; % because we do not need to plot and save figures here, we want to do cross validation time fast
        beh_par.SAVE_PRED_RESULTS = true; % so that eval_of_warp_cor can load correctly the predicted DVF
        ...
        parfor hrz_idx = 1:hppars.nb_hrz_val
            ...
            eval_results_pca_optim = eval_of_warp_corr( of_type_idx, im_par, OF_par, path_par, warp_par_h, pred_par_h, br_model_par, disp_par, beh_par, eval_results_pca_optim, time_signal_pred_results);
