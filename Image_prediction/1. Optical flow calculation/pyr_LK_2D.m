...
% This function computes the optical flow between I and J using the Lucas Kanade method.

    u_temp = cell(OF_par.nb_layers,1);
    [top_w, top_l] = size(pyr_I{OF_par.nb_layers});
    u_temp{OF_par.nb_layers} = zeros(top_w, top_l, 2, 'single');
        % u_temp{lyr_idx}(y,x,1) contains the x coordinate of the original guess at the layer lyr_idx and pixel (x,y)
    ...
    for lyr_idx = OF_par.nb_layers:(-1):1 % "-1" for decrementation
        ...
        if (lyr_idx>1)
            ...
            u_temp{lyr_idx-1} = 2*u_temp{lyr_idx}(Y,X,:);
                % pour éviter boucle dans u_temp{lyr_idx-1}(y, x, :) = 2*u_temp{lyr_idx}(aux(y), aux(x), :)
        end
