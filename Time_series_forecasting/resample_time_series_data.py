...
if resample_factor < 1:
    ...
    no_noise_data = copy.deepcopy(resampled_data)
...
if DISPLAY_FIGURES:
    ...
    f = plt.figure(1)
    plt.plot(t_new, resampled_data[crd_idx_zoom_plot, :t_max_zoom_new], "k.")  # z1
    # plt.title(new_data_plot_title)
    plt.xlabel("Time (s)", fontsize=16)
    plt.ylabel("z coordinate (mm)", fontsize=16)
    ...
    f2 = plt.figure(2)
    plt.plot(t_org, org_time_data[crd_idx_zoom_plot, :t_max_zoom_org], "k.")  # z1
    ...
    if resample_factor > 1:
        f3 = plt.figure(3)
        plt.plot(t_new, no_noise_data[crd_idx_zoom_plot, :t_max_zoom_new], "k.")  # z1
        # plt.title("Resampled data before noise addition")
        plt.xlabel("Time (s)", fontsize=16)
        plt.ylabel("z coordinate (mm)", fontsize=16)
        plt.xticks(fontsize=16)
        plt.yticks(fontsize=16)
    ...
    if DISPLAY_ALL_CRDS:
        ...
        coord_idx_dict = {0: "x", 1: "y", 2: "z"}

        for coord_idx in range(3):
            for mkr_idx in range(3):

                start_idx = 3 if resample_factor <= 1 else 4

                g1 = plt.figure(start_idx + 2 * mkr_idx + 1 + coord_idx * 6)
                ...
                g2 = plt.figure(start_idx + 2 * mkr_idx + 2 + coord_idx * 6)
