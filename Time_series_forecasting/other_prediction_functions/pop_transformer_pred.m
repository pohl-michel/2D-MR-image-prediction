function [Ypred, avg_pred_time, pred_loss_function] = pop_transformer_pred(path_par, pred_par, X, Y, run_idx)
% prediction with a population tranformer encoder model followed by a feed-forward layer.
% This function is merely a wrapper around the population_model_predict function in transformers_forecasting.py
% There are many similarities with transformer_pred() so there could be much refactoring here
% X: past data matrix 
...
% of the X and Y matrices inside load_pred_data_XY()
    ...
    pred_par.config_path = config_path;  % Rk: the transformer configuration (e.g., nb. of layers), is the same for all runs and horizons

    % Converting the structure containing the transformer parameters to a Python dictionary.
    pred_par_py = convert_to_py_dict(pred_par); % I need to review that

    % Reshaping the data matrix for use by the Python transformer code
    X_reshaped = reshape_data_matrix(X, n_features, pred_par.SHL, M); % Yes, put that function in a separate file!

    % Getting the test data (we don't test with the whole sequence to keep same experimental setup with other methods)
    ...
    [Ypred, avg_pred_time] = unpack_python_prediction(results); % Similarly, I need that in separate file
...
% casting pred_par values to types that can be understood in Python
    ...
    int_fields = ["batch_size", "num_epochs", "SHL", "d_model", ...
                  "nhead", "num_layers", "dim_feedforward", "final_layer_dim", "horizon", "seq_length", "n_features"];
